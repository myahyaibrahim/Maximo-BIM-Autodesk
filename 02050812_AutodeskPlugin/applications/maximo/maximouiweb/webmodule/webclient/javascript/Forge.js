...
IBM.LMV.ForgeViewer = function()
{
	...
	this.createViewer = function(
		...
		this.viewer = new Autodesk.Viewing.GuiViewer3D( this.ctrlViewer, {});
	...
	this.onDocumentLoad = function(  // onLoadCallback
		...
		if( geometryItems.length > 0 ) 
		{
			...
			this.viewer.loadModel( doc.getViewablePath(geometryItems[0]), options,  
							  function( d ) { _self.onModelLoad( d ) },
							  function( errorNum, errorMsg ) { _self.onLoadError( errorNum, errorMsg ) } );
			_selectionMgr.loadGUIDS(doc, geometryItems);
		}
    ...
    this.cancelMarkupMode = function() {
        ...
        if(markup != null) {
           if(markup.duringEditMode || markup.duringViewMode) {
               ...
               if(IBM.LMV.TOP_TOOLBAR)
               {
                   IBM.LMV.TOP_TOOLBAR.toolBarDiv.style.display = "";
                   IBM.LMV.TOP_TOOLBAR.isShown = true;
               }
           }
...
IBM.LMV.SelectionMgr = function(
	...
	this.fireSelectionChange = function(
		...
		var isolatedNodeDbId = _forgeViewer.viewer.getIsolatedNodes();
		var isolatedNode = isolatedNodeDbId.length > 0 ? true : false;
		var dbId = isolatedNodeDbId[0];

		if(!isolatedNode && _isAutoZoom && selection.length > 0 )
		{
		...
		else if (isolatedNode && isolatedNodeDbId.length > 0 )
		{
			this.zoomToContext(dbId);
		}
    ...
    this.hideInformationBox = function() {
        if(IBM.LMV.Markup.informationMessageBox != null) {
           IBM.LMV.Markup.informationMessageBox.setVisible( false );
        }
    }
	...
	this.loadGUIDS = function (doc, geometryItems) 
	{
		if( _docType != "rvt" && _docType != "zip"  )
		{
			_guidLoading = false;
		...
		var dbPath = geometryItems[0].findPropertyDbPath();
		var propDbPath = dbPath + "objects_ids.json.gz";
...
IBM.LMV.MessageBox.prototype.initialize = function()
{
	...
	scrollContainer.textContent = this.message;
