...
IBM.LMV.currentSelectedViewerState = null;
IBM.LMV.savedViewID = null;
...
IBM.LMV.Strings[ "ERR_REST_DELETE_VIEW" ]     = "The current view cannot be deleted because defects are associated with it. You can delete defects on the Defects tab, which is accessed on the Work tab.";
IBM.LMV.Strings[ "CANNOT_DELETE_VIEW" ]       = "View cannot be deleted";
...
IBM.LMV.toolBarExtension = function(
	...
	this.addZoomToModelControl = function( e )
	{
		var mainToolbar = _forgeViewer.viewer.getToolbar(true);
		...
		_forgeViewer.viewer.removeEventListener(Autodesk.Viewing.GEOMETRY_LOADED_EVENT);
	};
...
IBM.LMV.setCurrentSelectedViewerState = function() {
        var currentViewerState = IBM.LMV.viewer.getState();
        IBM.LMV.currentSelectedViewerState = currentViewerState;
}
...
IBM.LMV.SaveViewDlg = function (
	...
	this.saveView = function()
	{
        ...
        if(appId === "plusxasset" || appId === "plusxlocation") {
           var mboIdentifier = window.parent.document.querySelectorAll('[aria-readonly="true"]')[0].value;
           form = form + "&identifier" + "=" + mboIdentifier;
        } 
        else if(appId === "plusxwo") {
            var mboIdentifier = window.parent.document.querySelectorAll('[aria-readonly="true"]')[3].value;
            form = form + "&identifier" + "=" + mboIdentifier;
        }
	...
	this.makeSaveViewScreen = function(
		...
		cell.textContent = IBM.LMV.Strings.DLG_BTN_CREATE;
		cell.onclick   = function() { that.saveView( this ); };

		var cell         = row.insertCell();
		cell.className = "maxlmv_DlgBoxButton";
		cell.textContent = IBM.LMV.Strings.DLG_BTN_CLOSE;
		...
		cell.textContent   = this[ "BUILDINGMODEL" ].TITLE;
		
		cell             = row.insertCell();
		cell.className   = "maxlmv_DlgText";
		cell.style.width = "50%";
		cell.textContent   = IBM.LMV.title;
		
		cell             = row.insertCell();
		cell.className   = "maxlmv_DlgText";
		cell.style.width = "auto";
		cell.textContent   = this[ "BUILDINGMODEL" ].LOCATION;
		
		cell             = row.insertCell();
		cell.className   = "maxlmv_DlgText";
		cell.style.width = "50%";
		cell.textContent   = IBM.LMV.location;

		row              = dlgTable.insertRow( i++ );
		cell             = row.insertCell();
		cell.className   = "maxlmv_DlgHeading";
		cell.colSpan     = 4;
		cell.textContent   = IBM.LMV.Strings.DLG_LABEL_SAVE_VIEW;
        
        //add static label for asset/location number display, this is only available in asset/location application.
        var appId = window.parent.APPID;
        if(appId === "plusxasset" || appId === "plusxlocation") {
            var row = dlgTable.insertRow( i++ );
            this.displayCustomizedIdentifier(row);
        }
		row              = dlgTable.insertRow( i++ );
		cell             = row.insertCell();
		cell.colSpan     = 4;
		cell.className   = "maxlmv_DlgText";
		cell.textContent   = this[ "BIMLMVSAVEDVIEW" ].DESCRIPTION;
		...
		cell.colSpan     = 4;
		cell.className   = "maxlmv_DlgText";
		cell.textContent   = this[ "BIMLMVSAVEDVIEW" ].DESCRIPTION_LONGDESCRIPTION;
		...
		row                = dlgTable.insertRow( i++ );
		cell               = row.insertCell();
		cell.textContent     = this[ "BIMLMVSAVEDVIEW" ].SHARED;
		cell.colSpan       = 4;
		cell.className     = "maxlmv_DlgText";
    ...
    this.displayCustomizedIdentifier = function(row) {
        var appId = window.parent.APPID;
        if(appId === "plusxasset" || appId === "plusxlocation") {
            var cell         = row.insertCell();
            cell.colSpan     = 4;
            ...
            if(appId === "plusxasset") {
               cell.textContent   = IBM.LMV.Strings.ASSET_NUMBER+" "+ mboKey;
            } else {
               cell.textContent   = IBM.LMV.Strings.LOCATION_NUMBER+" "+ mboKey;
            }
        }
    }
	...
	this.onSaveView = function( request )
	{
		...
		else if (request.status == 200 && (appId === 'asset' || appId == 'location'))
		{    
			appId === 'asset' ? window.parent.sendEvent('SAVEVIEWSTATUSOK') : window.parent.sendEvent('SAVEVIEWSTATUSOKLOC');   
		}
		
        var data = JSON.parse( request.responseText );
        IBM.LMV.savedViewID = data.BIMLMVSAVEDVIEW ? data.BIMLMVSAVEDVIEW.Attributes.BIMLMVSAVEDVIEWID.content : '';	
        
        IBM.LMV.setCurrentSelectedViewerState();
		this.uninitialize();
	};
...
IBM.LMV.LoadViewDlg = function (
	...
	this.onDeleteButton = function(
		...
		//TODO MAXCI-1454 - is there a better way to handle eventsDisabled
		messageBox.addVisibilityListener((show) => {
			if (!show) {
				this.eventsDisabled = false;
			};
		});
	...
	this.lookupSavedViews = function() 
	{
        ...
        if(appId === "plusxasset" || appId === "plusxlocation") {
			var mboIdentifier = window.parent.document.querySelectorAll('[aria-readonly="true"]')[0].value;	            	
            url = url +  "&~identifier=" + mboIdentifier;
        } 
        else if(appId === "plusxwo") {
            var mboIdentifier = window.parent.document.querySelectorAll('[aria-readonly="true"]')[3].value;
            url = url +  "&~identifier=" + mboIdentifier;
	...
	this.onDelete = function( request )
	{
		...
		if( request.status != 200 )
		{
			var result = request.responseText.toUpperCase();
			if(result.includes('BIMFV0107E'))
			{
				var messageBox = new IBM.LMV.MessageBox( this.parent, IBM.LMV.Strings.ERR_REST_DELETE_VIEW, IBM.LMV.Strings.CANNOT_DELETE_VIEW );
				messageBox.setVisible( true );
				return;
			}
			else
			{
				var messageBox = new IBM.LMV.MessageBox( this.parent, request.responseText, IBM.LMV.Strings.ERR_REST );
				messageBox.setVisible( true );
				return;
			}
	...
	this.onHeadings = function( mbo, headings )
	{
		...
		this.header.cells[0].textContent = headings[ "DESCRIPTION" ];
		this.header.cells[1].textContent = headings[ "OWNER" ];
		this.header.cells[2].textContent = headings[ "DESCRIPTION_LONGDESCRIPTION" ];
		this.header.cells[3].textContent = headings[ "SHARED" ];
		this.populateList();
	};
	...
	this.populateList = function()
	{
		...
		for( var i = 0; i < this.viewList.length; i++ )
		{
			...
			cell.className = "maxlmv_DlgText";
			cell.textContent = this.viewList[i].DESCRIPTION;
			
			cell = row.insertCell( 1 );
			cell.className = "maxlmv_DlgText";
			cell.textContent = this.viewList[i].OWNER;
...
IBM.LMV.LoadViewDlg.prototype.setVisible = function(
	...
	if( show )
	{
		...
		var cell         = row.insertCell();
		cell.className = "maxlmv_DlgBoxButton";
		cell.textContent = IBM.LMV.Strings.DLG_BTN_APPLY;
...
//		this.container.style.height    = "auto";
		...
		for( var i = 0; i < attachments.length; i++ )
		{
			...
			{
				ref.textContent = attach.DOCUMENT;
			}
			else
			{
				ref.textContent = attach.DESCRIPTION;
			}
...
IBM.LMV.AttachmentDialog.prototype.initialize = function () 
{
   Autodesk.Viewing.UI.DockingPanel.prototype.initialize.call(this);
	...
	btn.textContent = IBM.LMV.Strings.DLG_BTN_CLOSE;
