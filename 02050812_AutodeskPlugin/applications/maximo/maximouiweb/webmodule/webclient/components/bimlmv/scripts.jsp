...
function loadControl(
	...
	vfFunctions.resizeCtrl = function(passArg) { };
...
function ViewerWrapper(
	...
	this.initialize = function(
		...
		<%if( appType ==  BIMViewer.TYPE_WORKORDER || appType ==  BIMViewer.TYPE_ASSET || appType ==  BIMViewer.TYPE_ASSET_CIVIL 
				|| appType ==  BIMViewer.TYPE_LOCATION || appType ==  BIMViewer.TYPE_LOCATION_CIVIL )
		{%> 
		...
		IBM.LMV.PATH_IMAGES      = "<%=IMAGE_PATH_MAS8%>bim/";
		IBM.LMV.Auth.contextRoot = "/rest";
        
        //TODO get skin build correct
        IBM.LMV.BIM_IMAGE_ROOT_PATH = IBM.LMV.PATH_IMAGES.replace("rtl/","");
	...
	this.loadFile = function(  
		...
		_forgeViewer.viewer.addEventListener( Autodesk.Viewing.GEOMETRY_LOADED_EVENT, 
									function() { IBM.LMV.toolBar.addZoomToModelControl() } );
	...
	this.reziseViewer = function(
		...
		height -= 25; // bottom status text height
...
function MaximoToolBar(
	...
	this.makeToolBar = function()
	{
        ...
        this.makeSavedViewSubToolbar(topToolBar);
	...
	this.makeModelSubToolbar = function(
		...
		<%if( appType ==  BIMViewer.TYPE_LOCATION || appType ==  BIMViewer.TYPE_LOCATION_CIVIL )
		{%>
	...
	this.makeRequestSubToolbar = function(
		...
		<%if(    appType ==  BIMViewer.TYPE_ASSET || appType ==  BIMViewer.TYPE_LOCATION ||
                 appType ==  BIMViewer.TYPE_ASSET_CIVIL || appType ==  BIMViewer.TYPE_LOCATION_CIVIL )
		{%>
			...
			ctrlCount++;
		<%}
		if(    appType ==  BIMViewer.TYPE_ASSET_CIVIL || appType ==  BIMViewer.TYPE_LOCATION_CIVIL )
 		{%>
 			<%-- Create a new defect --%>
 			var buttonCreateDef = new Autodesk.Viewing.UI.Button( "btnNewDef" );
 			buttonCreateDef.icon.style.backgroundImage = "url(" + "<%=BIM_IMAGE_PATH%>/tb_newTicket.png" + ")";
 			buttonCreateDef.setToolTip( IBM.LMV.Strings.create_defect_btn );
            <% if(appType ==  BIMViewer.TYPE_ASSET_CIVIL) { %>
            buttonCreateDef.onClick = function() { window.parent.sendEvent(  'CREATEDEF', '<%=bldgMdl.getRenderId()%>' ); };
            <% } else { %>
            buttonCreateDef.onClick = function() { window.parent.sendEvent(  'CREATEDEF_LOC', '<%=bldgMdl.getRenderId()%>' ); };
            <% } %> 
 			subToolbar.addControl( buttonCreateDef );
 			ctrlCount++;
 		<%}
	...
	this.makeSystemSubToolbar = function(
		...
		{%>
			...
			var buttonUpdateSystem = new Autodesk.Viewing.UI.Button( "<%=updateSystemId%>" );
			buttonUpdateSystem.icon.style.backgroundImage = "url(" + "<%=BIM_IMAGE_PATH%>/tb_editSystem.png" + ")";
			buttonUpdateSystem.setToolTip( "<%=strings.updateSystemBtn%>" );
			buttonUpdateSystem.onClick = function() { maximoIntf.maxSystemsUpdate(); };
			subToolbar.addControl( buttonUpdateSystem );
			ctrlCount++;
     	<%}%>
    ...
    this.makeSavedViewSubToolbar = function(
		toolBar
	) {
		<%if( appType ==  BIMViewer.TYPE_ASSET_CIVIL || appType ==  BIMViewer.TYPE_LOCATION_CIVIL )
		{%> 
			var subToolbar = new Autodesk.Viewing.UI.ControlGroup( "Maximo_top_toolabr_submenu_SAVEDVIEW" );

			buttonMaximoOpt = new Autodesk.Viewing.UI.Button("Maximo_toolabr_submenu.applySavedView");
            buttonMaximoOpt.icon.style.backgroundImage = "url(" + "<%=BIM_IMAGE_PATH%>/tb_layers.png" + ")";
			buttonMaximoOpt.setToolTip( IBM.LMV.Strings.APPLY_SAVED_VIEW );
			buttonMaximoOpt.onClick = function() { _self.displayRestoreStateDlg(); };
			subToolbar.addControl( buttonMaximoOpt );
            
            var buttonMaximoOpt = new Autodesk.Viewing.UI.Button("Maximo_toolabr_submenu.saveCurrentView");
			buttonMaximoOpt.icon.style.backgroundImage = "url(" + IBM.LMV.BIM_IMAGE_ROOT_PATH + "360_save.png )";
            buttonMaximoOpt.setToolTip( IBM.LMV.Strings.SAVE_CURRENT_VIEW );
	...
	this.makeWOSubToolbar = function(
		...
		<%if( appType ==  BIMViewer.TYPE_WORKORDER ||appType ==  BIMViewer.TYPE_ASSET_CIVIL || appType ==  BIMViewer.TYPE_LOCATION_CIVIL )
		{%> 
        ...
        <%-- Display dialog with list of markups associated with the current work order --%>
			...
			try
			{
				if( app.isSigOptionCheck( "BIM_SMU" ) ||appType ==  BIMViewer.TYPE_ASSET_CIVIL || appType ==  BIMViewer.TYPE_LOCATION_CIVIL )
				{%>
					var buttonMaximoOpt = new Autodesk.Viewing.UI.Button("Maximo_toolabr_submenu.restoreWorkView");
					buttonMaximoOpt.icon.style.backgroundImage = "url(" + IBM.LMV.BIM_IMAGE_ROOT_PATH + "360_redline_show.png )";
                    //TODO get the list of markups linked with defects on asset
                    <% if(appType ==  BIMViewer.TYPE_ASSET_CIVIL || appType ==  BIMViewer.TYPE_LOCATION_CIVIL) { %>
                    buttonMaximoOpt.setToolTip(IBM.LMV.Strings.show_defect_markups);
                    buttonMaximoOpt.onClick = function() { _self.showDefectMarkups(); };
                    <% } else { %>
			...
			try
			{
				if( app.isSigOptionCheck( "BIM_EMU" ) ||appType ==  BIMViewer.TYPE_ASSET_CIVIL || appType ==  BIMViewer.TYPE_LOCATION_CIVIL )
				{%>
					buttonMaximoOpt = new Autodesk.Viewing.UI.Button("Maximo_toolabr_submenu.createMarkup");
					buttonMaximoOpt.icon.style.backgroundImage = "url(" + IBM.LMV.BIM_IMAGE_ROOT_PATH + "360_redline.png )";
					buttonMaximoOpt.setToolTip( IBM.LMV.Strings.MARKUP_START_EDIT );
					buttonMaximoOpt.onClick = function() { _self.createMarkup("<%=appType%>","<%=bldgMdl.getRenderId()%>"); };
					subToolbar.addControl( buttonMaximoOpt );
				<%}
			}
			catch( MXException mxe )
